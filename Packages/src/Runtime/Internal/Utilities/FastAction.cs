...
namespace Coffee.UIParticleInternal
{
    ...
    internal class FastActionBase<T>
    {
        ...
        {
            // Callbacks can unregister themselves or add new callbacks. A removed
            // node is returned to a pool and cannot be used as the iteration cursor.
            var snapshot = InternalListPool<T>.Rent();
            try
            {
                foreach (var item in _delegates) snapshot.Add(item);
                for (var i = 0; i < snapshot.Count; i++)
                {
                    try { callback(snapshot[i]); }
                    catch (Exception e) { Debug.LogException(e); }
                }
            }
            finally { InternalListPool<T>.Return(ref snapshot); }
        }
    }
