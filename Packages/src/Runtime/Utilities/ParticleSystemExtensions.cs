...
namespace Coffee.UIParticleInternal
{
    internal static class ParticleSystemExtensions
    {
        ...
        public static void ValidateShape(this ParticleSystem self)
        {
            ...
            {
                // ShapeModule.scale returns a Vector3 copy; calling Set on that
                // temporary does not update the native particle system.
                var scale = shape.scale;
                if (Mathf.Approximately(scale.x, 0)) scale.x = 0.0001f;
                if (Mathf.Approximately(scale.y, 0)) scale.y = 0.0001f;
                if (Mathf.Approximately(scale.z, 0)) scale.z = 0.0001f;
                shape.scale = scale;
            }
        }
        ...
        {
            // List.Sort mutates self while comparing. Preserve original order for
            // ties, and always return zero when comparing an item with itself.
            var originalOrder = new Dictionary<ParticleSystem, int>(self.Count);
            for (var i = 0; i < self.Count; i++)
                if (self[i] != null && !originalOrder.ContainsKey(self[i])) originalOrder.Add(self[i], i);
            self.Sort((a, b) =>
            {
                if (a == b) return 0;
                if (a == null) return -1;
                ...
                if (aRenderer == null || bRenderer == null)
                    return (aRenderer != null).CompareTo(bRenderer != null);
                ...
                if (sortByMaterial)
                {
                    var materialOrder = aMat.GetHashCode().CompareTo(bMat.GetHashCode());
                    return materialOrder != 0 ? materialOrder : originalOrder[a].CompareTo(originalOrder[b]);
                }

                if (aMat.renderQueue != bMat.renderQueue)
                {
                    return aMat.renderQueue.CompareTo(bMat.renderQueue);
                }

                // Sorting layer: ascending
                if (aRenderer.sortingLayerID != bRenderer.sortingLayerID)
                {
                    return SortingLayer.GetLayerValueFromID(aRenderer.sortingLayerID).CompareTo(
                ...
                {
                    return aRenderer.sortingOrder.CompareTo(bRenderer.sortingOrder);
                }
