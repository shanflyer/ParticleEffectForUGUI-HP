...
#region Bloom
        ...
        int GetBloomQualityLevel()
        {
            int qualityCount = QualitySettings.names.Length;
            if (qualityCount <= 1)
                return 1;

            float normalizedQuality = (float)QualitySettings.GetQualityLevel() / (qualityCount - 1);
            return normalizedQuality < 1.0f / 3.0f ? 3
                 : normalizedQuality < 2.0f / 3.0f ? 2
                 : 1;
        }

        void SetupBloom(CommandBuffer cmd, RTHandle source, Material uberMaterial)
        {
            int bloomQualityLevel = GetBloomQualityLevel();
            // Low quality starts at quarter-res to reduce the cost of the largest passes.
            int downres = bloomQualityLevel >= 3 ? 2 : 1;
            ...
            // High / medium / low quality levels use at most 4 / 3 / 2 Bloom mips.
            int qualityMaxIterations = bloomQualityLevel <= 1 ? 4
                                     : bloomQualityLevel <= 2 ? 3
                                     : 2;
            int mipCount = Mathf.Clamp(iterations, 1, Mathf.Min(m_Bloom.maxIterations.value, qualityMaxIterations));
            ...
            CoreUtils.SetKeyword(bloomMaterial, ShaderKeywordStrings.BloomHQ, false);
            ...
            // Bloom.shader and UberPost only retain the LQ Bloom path.
            CoreUtils.SetKeyword(uberMaterial, ShaderKeywordStrings.BloomHQ, false);
            CoreUtils.SetKeyword(uberMaterial, ShaderKeywordStrings.BloomHQDirt, false);
            CoreUtils.SetKeyword(uberMaterial, ShaderKeywordStrings.BloomLQDirt, false);
            CoreUtils.SetKeyword(uberMaterial, ShaderKeywordStrings.BloomLQ, true);
        }
