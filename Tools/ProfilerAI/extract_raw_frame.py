...
def source_profile_from_export(export_dir: Path) -> Path:
    for name in ("ai_profiler_summary.json", "ai_profiler_digest.json"):
        path = export_dir / name
        if not path.exists():
            continue
        data = read_json(path)
        source = data.get("sourceProfilePath")
        if source:
            return resolve_project_path(source).resolve()

    fallback = export_dir / "profile.data"
    if fallback.exists():
        return fallback.resolve()

    raise FileNotFoundError("Could not find sourceProfilePath in export metadata or profile.data in " + str(export_dir))


def default_output_dir(export_dir: Path, first_frame: int, last_frame: int) -> Path:
    name = f"frame_{first_frame}" if first_frame == last_frame else f"frames_{first_frame}_{last_frame}"
    return export_dir / "raw_frames" / name


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate full raw TSV data for selected frames from an existing ProfilerAI export folder.")
    parser.add_argument("--export-dir", required=True, help="Existing ProfilerAI export folder, project-relative or absolute.")
    ...
    parser.add_argument("--last-frame", type=int, help="Last frame index to export.")
    parser.add_argument("--out", help="Output folder. Defaults to <export-dir>/raw_frames/frame_N.")
    ...
    export_dir = resolve_project_path(args.export_dir).resolve()
    if not export_dir.exists():
        raise FileNotFoundError(export_dir)

    profile = source_profile_from_export(export_dir)
    if not profile.exists():
        raise FileNotFoundError(profile)

    out_dir = resolve_project_path(args.out).resolve() if args.out else default_output_dir(export_dir, first_frame, last_frame)
