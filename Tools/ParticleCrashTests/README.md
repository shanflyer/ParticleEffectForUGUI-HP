...
```powershell
C:\Python314\python.exe Tools/ParticleCrashTests/run_extended.py
```

使用本机 Unity 2022.3.49f1 附带的 C# 编译器、现存 Bee 引用参数和 Windows SDK FXC。仅写入 `TempDiag/crash_fix_compile/`；不启动 Unity，不进入 Play Mode，不创建图形上下文，不运行粒子/显卡/内存压力场景。

- 编译编辑器配置下的 URP、粒子库、项目运行时代码和两个编辑器程序集。
- 编译 Player 配置下的 URP、粒子库和项目运行时代码；这不是打包或运行 Player。
- 编译 Overdraw HLSL 为 cs_5_0；FXC 忽略 Unity 专用的 `#pragma kernel`，不执行 GPU 工作。
- `UpdaterHarness.cs`：实际调度器源码 + 托管替身，30 项，含局部缓存失效、Primary 交接、按工作量分配相位、共享组可见性及公开 Dirty API 的组传播。公开 API 方法体也从实际 UIParticle 源码抽取。
- `LogicHarness.cs`：实际计时器、回调分发、采集器源码 + 托管替身，26 项，含低帧率隔帧、高帧率限频、相位变化下时间守恒、数值采样零分配、按需计时生命周期和 25 列 CSV 验证。
...
- `ReadbackHarness.cs`：从实际 Overdraw 文件抽取计数器类，仅改类访问修饰符以独立编译，GPU 资源用托管替身替代，5 项。

合计 **98 个不重复的修复后检查**。`run_offline.py` 的 11 项初轮调度检查是上述 30 项的子集，不能重复计数。初轮原代码的 7 项失败、本轮原工具代码的 7 项失败是预期的回归对照，不是修改后失败。
